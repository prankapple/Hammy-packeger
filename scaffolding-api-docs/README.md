...
You have to manually configure scratch-storage to know where to fetch files from. If you want to load projects from projects.Blox.site, you would do:

```js
const storage = scaffolding.storage;
storage.addWebStore(
  [storage.AssetType.Project],
  (asset) => `https://projects.Blox.site/api/projects/getPublished?type=file&id=${asset.assetId}`
);
storage.addWebStore(
...
If you configured scratch-storage to load projects from projects.Blox.site, you can use:
